...
export class HttpSc2ArcadeLobbiesService extends HttpBase {
  ...
  getActiveLobbies(httpRequestOptions?: HttpRequestOptions): Observable<HttpLobby[]> {

    const getActiveLobbiesRequest$ = this.http.get<HttpLobby[]>(`${SC2_ARCADE_LOBBIES()}active`);
